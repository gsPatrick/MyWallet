...
/**
 * Investment Health Tab Component
 * ================================
 * Diagnostic dashboard for investment data sources
 */
...
import styles from './InvestmentHealthTab.module.css';

export default function InvestmentHealthTab() {
    ...
    const runDiagnostics = async () => {
        ...
        setError(null);
        try {
            // The axios interceptor in api.js already returns response.data
            // So the API response { success: true, data: {...} } comes directly
            const apiResponse = await investmentsAPI.getHealthDiagnostics();
            console.log('API Response:', apiResponse);

            if (apiResponse && apiResponse.success && apiResponse.data) {
                setDiagnostics(apiResponse.data);
            } else if (apiResponse && apiResponse.data) {
                // Fallback if success isn't boolean
                setDiagnostics(apiResponse.data);
            } else {
            ...
            setError(err?.error || err?.message || 'Erro ao executar diagnósticos');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className={styles.container}>
            <div className={styles.header}>
                <div>
                    <h2>🔬 Diagnóstico de Investimentos</h2>
                                ...
                                >
                                    <div className={styles.serviceInfo}>
                                        ...
                                        <div className={styles.serviceName}>
                                            <h4>{getServiceName(diag.service)}</h4>
